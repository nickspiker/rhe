...
//!
//! The tutor window (and any future status UI) renders into a softbuffer pixel buffer. The renderer module below is the platform-agnostic handle; each platform has its own implementation underneath.
//!
//! Only Linux and macOS are supported this pass. Windows/Redox/Android renderers from photon are not carried over.
//!
//! Layout discipline: no pixel-unit dimensions anywhere except 1px hairlines. Every other size derives from `span = 2·w·h/(w+h)` times an `ru` zoom factor. See `scale()`.
...
/// Harmonic-mean scale invariant. Responsive, orientation-agnostic — both landscape and portrait of the same diagonal pick the same span.
#[inline]
